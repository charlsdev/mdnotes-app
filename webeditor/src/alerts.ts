// Plugin de ProseMirror para VIVO: pinta los blockquotes `[!TYPE]` como callouts
// con color, OCULTA el marcador `[!TYPE]` y muestra un encabezado con ícono +
// etiqueta (Nota/Consejo/…), igual que VER.
import { $prose } from '@milkdown/kit/utils';
import { Plugin, PluginKey } from '@milkdown/kit/prose/state';
import { Decoration, DecorationSet } from '@milkdown/kit/prose/view';

// Tolera un `\` de escape por delante (`\[!NOTE]`).
...
function headerEl(type: string): HTMLElement {
  const { label, icon } = ICONS[type];
  const el = document.createElement('span');
  el.className = 'gh-alert-header';
  el.contentEditable = 'false';
  el.innerHTML = `<svg viewBox="0 0 16 16" width="15" height="15" fill="currentColor"><path d="${icon}"/></svg><span>${label}</span>`;
  return el;
}
      ...
      props: {
        decorations(state) {
          ...
          state.doc.descendants((node, pos) => {
            ...
            const m = para.textContent.match(MARKER_RE);
            if (!m || m.index === undefined) return;
            const type = m[2].toUpperCase();

            // Clase de color en el blockquote.
            decos.push(
              Decoration.node(pos, pos + node.nodeSize, { class: `gh-alert gh-alert-${type.toLowerCase()}` })
            );

            // Posición del marcador dentro del primer párrafo: blockquote(pos) →
            // +1 (contenido) +1 (párrafo) = pos+2; + índice del match.
            const from = pos + 2 + m.index;
            const to = from + m[0].length;
            decos.push(Decoration.inline(from, to, { class: 'gh-alert-marker' }));
            decos.push(
              Decoration.widget(from, () => headerEl(type), {
                side: -1,
